...
class Analyzer:
    ...
    def __init__(self, audio_path, bpm, fps=60):
        ...
        self.pitch_detector = es.PitchYin(
    ...
    def get_audio_data(self):
        ...
        for start_sample in range(0, len(self.amplitude_envelope), self.samples_per_frame):
            ...
            raw_frame_samples = self.audio[
                start_sample:end_sample
            ]

            if len(raw_frame_samples) < self.samples_per_frame:
                raw_frame_samples = np.pad(
                    raw_frame_samples,
                    (0, self.samples_per_frame - len(raw_frame_samples))
                )

            pitch_hz, pitch_confidence = self.pitch_detector(
                raw_frame_samples
            )
            ...
            frame = {
                ...
                "pitch_hz": (
                    round(float(pitch_hz), 2)
                    if pitch_confidence > 0
                    else None
                ),
                "pitch_confidence": round(float(pitch_confidence), 2)
            }
